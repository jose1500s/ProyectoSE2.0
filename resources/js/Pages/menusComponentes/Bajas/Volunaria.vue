  ...
  methods: {
    ...
    registrarVoluntaria() {
      ...
      if (
        ...
        const data = {
          periodo: this.BVperiodo,
          año: this.BVaño,
          carrera: this.BVcarrera,
          hombres: this.BVhombres,
          mujeres: this.BVmujeres
        };
        this.$inertia.post("/registro-baja-voluntaria", data, {
          preserveState: true,
          preserveScroll: true,
          onSuccess: () => {
            this.productDialog = false;
    ...
    editarVoluntaria() {
      ...
      if (
        ...
        this.$inertia.post(`/editar-baja-voluntaria/${this.product.id}`, data2, {
          preserveState: true,
          preserveScroll: true,
          onSuccess: () => {
    ...
    editProduct(product) {
      ...
      this.BVperiodo = this.product.periodo;
      this.BVaño = this.product.año;
      this.BVcarrera = this.product.carrera;
      this.BVhombres = this.product.hombres;
      this.BVmujeres = this.product.mujeres;
    ...
    deleteProduct() {
      ...
      this.$inertia.post(`/eliminar-baja-voluntaria/${this.product.id}`, data2, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
    ...
    deleteSelectedProducts() {
      ...
      this.$inertia.post("/eliminar-bajas-voluntarias", data, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
    ...
    subirExcel() {
      ...
      if (input.files[0].type != "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet") {
        ...
        readXlsxFile(input.files[0]).then((rows) => {
          ...
          for (let i = 0; i < this.datosExcel.length; i++) { this.datosExcel[i][6] = "voluntaria" }
...
>>>>>>> 110d6c1665173eac4f7eaa1c4347a0e9cf341702
  ...
  <Dialog v-model:visible="productDialog" :breakpoints="{ '960px': '75vw', '640px': '85vw' }" :style="{ width: '25vw' }"
    ...
    <div class="field">
      <form @submit.prevent="registrarVoluntaria">
        <!-- select con opciones -->

        <div class="field col-12 md:col-3 mt-3">
          <label for="minmax">Periodo</label>
          <Dropdown v-model="BVperiodo" :options="cuatriLista" optionLabel="name" optionValue="code"
            placeholder="Seleccione" />
        </div>
        <div class="field col-12 md:col-3">
          <label for="minmax">Año</label>
          <InputNumber inputId="minmax" v-model="BVaño" mode="decimal" :min="0" :max="10000" :showButtons="true" />
        </div>


        <div class="field col-12 md:col-3">
          <label for="minmax">Carrera</label>
          <Dropdown v-model="BVcarrera" :options="carrerasLista" optionLabel="name" optionValue="code" :filter="true"
            placeholder="Seleccione" />
        </div>

        <div class="field col-12 md:col-3">
          <label for="minmax">Hombres</label>
          <InputNumber inputId="minmax" v-model="BVhombres" mode="decimal" :min="0" :max="10000" :showButtons="true" />
        </div>

        <div class="field col-12 md:col-3">
          <label for="minmax">Mujeres</label>
          <InputNumber inputId="minmax" v-model="BVmujeres" mode="decimal" :min="0" :max="10000" :showButtons="true" />
        </div>
  ...
  <section class="bg-white" id="tablaEgreso">
    <div class="mx-auto max-w-screen-xl px-4 sm:px-6 lg:px-8 p-[20px]">
      ...
      <DataTable :value="voluntaria" :paginator="true" class="p-datatable-customers" :rows="7" ref="dt"
      ...
      <Dialog header="Editar Baja" v-model:visible="editDialog" :breakpoints="{ '960px': '75vw', '75vw': '85vw' }"
        ...
        <div class="p-fluid p-formgrid p-grid">
          <form @submit.prevent="editarVoluntaria">
            ...
            <div class="field col-12 md:col-3">
              <label for="minmax">Hombres</label>
              <InputNumber inputId="minmax" v-model="BVhombres" mode="decimal" :min="0" :max="10000"
                :showButtons="true" />
            </div>

            <div class="field col-12 md:col-3">
              <label for="minmax">Mujeres</label>
              <InputNumber inputId="minmax" v-model="BVmujeres" mode="decimal" :min="0" :max="10000"
                :showButtons="true" />
            </div>
