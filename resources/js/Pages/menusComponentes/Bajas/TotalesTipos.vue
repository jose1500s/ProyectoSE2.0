...
<<<<<<< HEAD:resources/js/Pages/menusComponentes/Bajas/TotalesTipos.vue
  data() {
    return {
      filters: {
        tipo_baja: { value: null, matchMode: FilterMatchMode.IN },
      ...
      carrerasLista: [
        ...
        { name: "Académica", code: "Académica" },
        { name: "Voluntaria", code: "Voluntaria" },
        { name: "Administrativa", code: "Administrativa" },
      ],
...
>>>>>>> 110d6c1665173eac4f7eaa1c4347a0e9cf341702:resources/js/Pages/menusComponentes/Matricula/Matricula.vue
      if (
        ...
        this.$inertia.post("/registro-baja-total-tipos", data, {
          preserveState: true,
          preserveScroll: true,
          onSuccess: () => {
            this.productDialog = false;
...
>>>>>>> 110d6c1665173eac4f7eaa1c4347a0e9cf341702:resources/js/Pages/menusComponentes/Matricula/Matricula.vue
    ...
    deleteProduct() {
      ...
      this.$inertia.post(`/eliminar-baja-total-tipos/${this.product.id}`, data2, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
    ...
    deleteSelectedProducts() {
      ...
      this.$inertia.post("/eliminar-bajas-totales-tipos", data, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
    ...
    filtrarCarreras() {
      const carreras = [
        "Temporal", "Voluntaria", "Académica", "Administrativa"
      ];
      return carreras;
    },
    filtrarPeriodo() {
    ...
    limpiarFiltros() {
      ...
      this.filters.tipo_baja.value = null;
      this.filters.año.value = null;
      this.filters.periodo.value = null;
      this.$refs.dt.filter(this.filters, "tipo_baja");
      ...
      if (input.files[0].type != "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet") {
        ...
        readXlsxFile(input.files[0]).then((rows) => {
          ...
          if (
            ...
            this.columnasExcel[3] != "Tipo de Baja" ||
    ...
    importarExcel() {
      ...
      this.$inertia.post("/importar-excel-bajas-totales-tipos", data, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
...
<template>
  ...
  <Dialog v-model:visible="productDialog" :breakpoints="{ '960px': '75vw', '640px': '85vw' }" :style="{ width: '25vw' }"
    ...
    <div class="field">
      <form @submit.prevent="registrarTemporal">
        <!-- select con opciones -->
    ...
    <div class="mx-auto max-w-screen-xl px-4 sm:px-6 lg:px-8 p-[20px]">
      <div class="text-center mb-5">
        <div class="flex gap-5 justify-center flex-wrap">
          ...
          <MultiSelect v-model="filters.tipo_baja.value" :options="filtrarCarreras()" placeholder="Tipo de Baja"
            display="chip" />
          <Button icon="pi pi-times" label="Limpiar" @click="limpiarFiltros()" />
        </div>
      </div>

      <DataTable :value="totalesTipos" :paginator="true" class="p-datatable-customers" :rows="7" ref="dt"
...
>>>>>>> 110d6c1665173eac4f7eaa1c4347a0e9cf341702:resources/js/Pages/menusComponentes/Matricula/Matricula.vue
            ...
            <div class="field col-12 md:col-3">
              <label for="minmax">Hombres</label>
              <InputNumber inputId="minmax" v-model="BThombres" mode="decimal" :min="0" :max="10000"
                :showButtons="true" />
            </div>

            <div class="field col-12 md:col-3">
              <label for="minmax">Mujeres</label>
              <InputNumber inputId="minmax" v-model="BTmujeres" mode="decimal" :min="0" :max="10000"
                :showButtons="true" />
            </div>
  ...
  <Dialog header="Gráfica dinámica" v-model:visible="displayResponsive" 
    ...
    <!-- contenido del dialog/model desde aqui... -->
    <div class="w-full" id="contenedorGrafica">
      <GraficaTipos :data="selectedProducts" />
    </div>
    <template #footer>
      <Button label="Cerrar" icon="pi pi-check" @click="closeResponsive" autofocus />
      <!-- boton para guardar la grafica como img -->
