  ...
  methods: {
    ...
    subirExcel() {
      ...
      if (input.files[0].type != "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet") {
        ...
        readXlsxFile(input.files[0]).then((rows) => {
          ...
          if (
            this.columnasExcel[1] != "Ruta" ||
            this.columnasExcel[2] != "Lugares Disponibles" ||
            this.columnasExcel[3] != "Lugares Pagados" ||
            this.columnasExcel[4] != "Periodo" ||
            this.columnasExcel[5] != "Turno"
          ) {
    ...
    importarExcel() {
      ...
      for (let i = 0; i < this.datosExcel.length; i++) {
        datosInsertar.push({
          ruta: this.datosExcel[i][1],
          lugares_disp: this.datosExcel[i][2],
          pagados: this.datosExcel[i][3],
          cuatrimestre: this.datosExcel[i][4],
          turno: this.datosExcel[i][5],
        })
      }
      ...
      this.$inertia.post("/importar-excel-rutas", data, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
  ...
  data() {
    return {
      ...
      columnasPreviewExcel: [
        { name: "ID", code: "0" },
        { name: "Ruta", code: "1" },
        { name: "Lugares Disponibles", code: "2" },
        { name: "Lugares Pagados", code: "3" },
        { name: "Periodo", code: "4" },
        { name: "Turno", code: "5" },
      ],
      datosExcel: [],
...
<template>
  ...
  <section class="bg-white" id="tablaIngreso">
    <div class="mx-auto max-w-screen-xl px-4 sm:px-6 lg:px-8 p-[20px]">
      ...
      <DataTable exportFilename="Rutas" :value="rutas" :paginator="true" class="p-datatable-costumers" :rows="7" ref="dt"
      ...
      <Dialog v-model:visible="productDialog" :breakpoints="{ '960px': '75vw', '640px': '85vw' }"
        ...
        <div class="field">
          <form @submit.prevent="registrarRutas">
            <!-- select con opciones -->
            <Dropdown v-model="RUruta" :options="rutasLista" optionLabel="name" optionValue="code" :filter="true"
              placeholder="Ruta" class="!mt-3" />

            <div class="field col-12 md:col-3">
              <Dropdown v-model="RUcuatri" :options="cuatrisLista" optionLabel="name" optionValue="code"
                placeholder="Periodo" class="!mt-3" />
            </div>

            <div class="field col-12 md:col-3">
              <Dropdown v-model="RUturno" :options="turnosLista" optionLabel="name" optionValue="code" placeholder="Turno"
                class="!mt-3" />
            </div>

            <div class="field col-12 md:col-3">
              <label for="minmax">Lugares Disponibles</label>
              <InputNumber inputId="minmax" v-model="RUlugares" mode="decimal" :min="0" :max="10000"
                :showButtons="true" />
            </div>

            <div class="field col-12 md:col-3 mt-3">
              <label for="minmax">Lugares Pagados</label>
              <InputNumber inputId="minmax" v-model="RUpagados" mode="decimal" :min="0" :max="10000"
                :showButtons="true" />
            </div>
      ...
      <Dialog header="Editar Lugares" v-model:visible="editDialog" :breakpoints="{ '960px': '75vw', '75vw': '85vw' }"
        ...
        <div class="p-fluid p-formgrid p-grid">
          <form @submit.prevent="editarRutas">
            ...
            <div class="p-field p-col-12 p-md-12">
              ...
              <Dropdown v-model="Eruta" :options="rutasLista" optionLabel="name" optionValue="code" :filter="true"
                placeholder="Ruta" class="!mt-3" />
            </div>
