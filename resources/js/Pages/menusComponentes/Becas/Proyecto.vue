  ...
  methods: {
    ...
    registrarBecaPP() {
      ...
      if (
        ...
        this.$inertia.post("/registro-beca-por-proyecto", data, {
          preserveState: true,
          preserveScroll: true,
          onSuccess: () => {
            this.productDialog = false;
    ...
    editarBecaPP() {
      ...
      if (
        ...
        this.$inertia.post(`/editar-beca-por-proyecto/${this.product.id}`, data2, {
          preserveState: true,
          preserveScroll: true,
          onSuccess: () => {
    ...
    deleteProduct() {
      ...
      this.$inertia.post(`/eliminar-beca-por-proyecto/${this.product.id}`, data2, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
    ...
    deleteSelectedProducts() {
      ...
      this.$inertia.post("/eliminar-becas-por-proyectos", data, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
    ...
    subirExcel() {
      ...
      if (input.files[0].type != "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet") {
        ...
        readXlsxFile(input.files[0]).then((rows) => {
          ...
          for (let i = 0; i < this.datosExcel.length; i++) { this.datosExcel[i][6] = "proyecto"}
...
<template>
  ...
  <Dialog v-model:visible="importExcelDialog" :breakpoints="{ '1260px': '75vw', '640px': '85vw' }"
    ...
    >
      <div class="field">
        <form @submit.prevent="registrarBecaPP">
          <!-- select con opciones -->
