...
import GraficaBecas from "./GraficaBecas.vue";
// PrimeVue
...
export default {
  ...
  methods: {
    ...
    saveImage() {
      ...
      link.download = "Gráfica Becas.png";
      link.href = imagen;
      link.click();
    },
    registrarBecaAC() {
      ...
      if (
        ...
        const data = {
          ...
          año: this.BTaño,
          carrera: this.BTcarrera,
          hombres: this.BThombres,
          mujeres: this.BTmujeres
        };
        this.$inertia.post("/registro-beca-apoyo-cardenal", data, {
          preserveState: true,
          preserveScroll: true,
          onSuccess: () => {
            this.productDialog = false;
    ...
    editarBecaAC() {
      ...
      if (
        ...
        this.$inertia.post(`/editar-beca-apoyo-cardenal/${this.product.id}`, data2, {
          preserveState: true,
          preserveScroll: true,
          onSuccess: () => {
    ...
    editProduct(product) {
      ...
      this.BTcarrera = this.product.carrera;
    ...
    deleteProduct() {
      ...
      this.$inertia.post(`/eliminar-beca-apoyo-cardenal/${this.product.id}`, data2, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
    ...
    deleteSelectedProducts() {
      ...
      this.$inertia.post("/eliminar-becas-apoyo-cardenales", data, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
    ...
    subirExcel() {
      ...
      if (input.files[0].type != "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet") {
        ...
        readXlsxFile(input.files[0]).then((rows) => {
          ...
          for (let i = 0; i < this.datosExcel.length; i++) { this.datosExcel[i][6] = "apoyo"}
    ...
    importarDataExcelBecas() {
      ...
      this.$inertia.post("/importar-excel-becas", data, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
...
<template>
  ...
  <Dialog v-model:visible="importExcelDialog" :breakpoints="{ '1260px': '75vw', '640px': '85vw' }"
    ...
    <div class="max-w-[50%] m-auto p-7">
      <Button label="Importar" @click="importarDataExcelBecas" id="btnImportarExcel"
    ...
    >
      <div class="field">
        <form @submit.prevent="registrarBecaAC">
          <!-- select con opciones -->
    ...
    <section class="bg-white" id="tablaEgreso">
      <div class="mx-auto max-w-screen-xl px-4 sm:px-6 lg:px-8 p-[20px]">
        ...
        <div class="w-full" id="contenedorGrafica">
          <GraficaBecas :data="selectedProducts" />
        </div>
        <template #footer>
        ...
        >
          <div class="p-fluid p-formgrid p-grid">
            <form @submit.prevent="editarBecaAC">
