  ...
  methods: {
    ...
    registrarBecaCA() {
      ...
      if (
        ...
        this.$inertia.post("/registro-beca-cardenal-azul", data, {
          preserveState: true,
          preserveScroll: true,
          onSuccess: () => {
            this.productDialog = false;
    ...
    editarBecaCA() {
      ...
      if (
        ...
        this.$inertia.post(`/editar-beca-cardenal-azul/${this.product.id}`, data2, {
          preserveState: true,
          preserveScroll: true,
          onSuccess: () => {
    ...
    deleteProduct() {
      ...
      this.$inertia.post(`/eliminar-beca-cardenal-azul/${this.product.id}`, data2, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
    ...
    deleteSelectedProducts() {
      ...
      this.$inertia.post("/eliminar-becas-cardenal-azules", data, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
    ...
    subirExcel() {
      ...
      if (input.files[0].type != "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet") {
        ...
        readXlsxFile(input.files[0]).then((rows) => {
          ...
          for (let i = 0; i < this.datosExcel.length; i++) { this.datosExcel[i][6] = "azul"}
...
<template>
  ...
  <Dialog v-model:visible="importExcelDialog" :breakpoints="{ '1260px': '75vw', '640px': '85vw' }"
    ...
    >
      <div class="field">
        <form @submit.prevent="registrarBecaCA">
          <!-- select con opciones -->
      ...
      <div class="mx-auto max-w-screen-xl px-4 sm:px-6 lg:px-8 p-[20px]">
        ...
        >
          <div class="p-fluid p-formgrid p-grid">
            <form @submit.prevent="editarBecaCA">
